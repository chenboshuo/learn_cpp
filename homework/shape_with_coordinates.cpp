...
class Circle : public Shape {
 public:
  ...
  double area() { return PI * pow(redius, 2); }
  double perimeter() { return 2 * PI * redius; }
...
class Square : public Shape {
 public:
  Square(double x, double y, double vertex_x, double vertex_y) : Shape(x, y) {
    edge = fabs(x - vertex_x) * 2;
  };

  double area() { return pow(edge, 2); }

  double perimeter() { return 4 * edge; }
...
int main(int argc, char const *argv[]) {
  ...
  cout << "Plese input the center of a circle cordinates x y,and the redius r"
       << endl;
  cin >> x0 >> y0 >> r;
  Circle c(x0, y0, r);
...
/*
  ...
  Please input the center of a squarecordinates x,yand one of the vertex
  squarecordinates x0 y0 0 0 2 2 The area of square of the square is: 16 The
  perimeter of the circle is: 16
 */
